...
from lib.callbacks import update_obj, Node
from lib.robot import validate_robot

def _distance(a : Node, b : Node):
    return np.linalg.norm(np.array(a.pose()) - np.array(b.pose()))
...
def _is_collision(pos : Node, objects):
    ...
    """
    probe_sphere = Sphere(radius=objects["start"].radius * 3)
    update_obj(probe_sphere, pos.pose())

    # Check with Panda
    if "panda" in objects and objects["panda"].iscollided(objects["panda"].q, probe_sphere):
        return True

    # Check with Boxes
    if "box" in objects:
        for b in objects["box"]:
            if probe_sphere.iscollided(b):
                return True
...
def _find_robot_path(env, end_node, objects, radius = 0.04):
    path_nodes = []
    node = end_node
    node = validate_robot(env, objects, node)
    while node:
...
def find_tree(env, bounds, objects, step_size=0.1):
    """
    ...
    dest_node = Node(objects["dest"].T[0,3], objects["dest"].T[1,3], objects["dest"].T[2,3])
    nodes = [start_node]
    ...
    while not objects["dest"].iscollided(spheres[-1]):
        ...
        if not _is_collision(new_node, objects) and not _is_connection_in_collision(nearest, new_node, objects):
            ...
            env.add(spheres[-1])
            if _distance(new_node, dest_node) <= 0.25:
                if new_node.cost + _distance(new_node, dest_node) < c_best:
                    best_goal_node = Node(dest_node.x, dest_node.y, dest_node.z, new_node.rot_matrix,
                                          new_node, new_node.cost + _distance(new_node, dest_node))
                    c_best = best_goal_node.cost
                    # print(f"Better path found at iteration {i}, cost: {c_best:.2f}")
    if best_goal_node:
        # path_nodes = _show_path(env, best_goal_node, radius=objects["start"].radius)
        path_nodes = _find_robot_path(env, best_goal_node, objects, radius=objects["start"].radius)
